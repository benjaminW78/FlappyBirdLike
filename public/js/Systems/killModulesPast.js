var canvasConf = require("../modules/configCanvas");
var eventsController = require("../modules/EventController");

    var getIdPlayer = Game.Puppets.find('score');
        player = Game.Puppets.getComponents(getIdPlayer[0])[0];
    var getIdCamera = Game.Puppets.find('targetCamera');
        camera = Game.Puppets.getComponents(getIdCamera[0])[0];

Game.Puppets.system("garbageCollector",function(position,size,collider,polygone,entity){

        if(player.position.y!==undefined && collider.type !== player.collider.type){
             
            if(position.y>player.position.y+1000)
            {
                Game.Puppets.removeEntity(entity);
            }
        }
        

},{components : ["position",'size','collider',"polygone"]});

module.exports = this;
